...
func (s *suite) TestGetArchive(c *gc.C) {
	ch := entitytesting.Repo.CharmArchive(c.MkDir(), "wordpress")
...
func (s *suite) TestUploadArchiveWithCharm(c *gc.C) {
	path := entitytesting.Repo.CharmArchivePath(c.MkDir(), "wordpress")
	...
	// revision.
	path = entitytesting.Repo.CharmArchivePath(c.MkDir(), "mysql")
...
func (s *suite) prepareBundleCharms(c *gc.C) {
	...
	err := s.store.AddCharmWithArchive(
		...
		entitytesting.Repo.CharmArchive(c.MkDir(), "wordpress"))
	c.Assert(err, gc.IsNil)
	err = s.store.AddCharmWithArchive(
		charm.MustParseReference("utopic/mysql-47"),
		entitytesting.Repo.CharmArchive(c.MkDir(), "mysql"))
	c.Assert(err, gc.IsNil)
}

func (s *suite) TestUploadArchiveWithBundle(c *gc.C) {
	s.prepareBundleCharms(c)
	path := entitytesting.Repo.BundleArchivePath(c.MkDir(), "wordpress-simple")
	// Post the archive.
...
func (s *suite) TestUploadCharmErrorUploading(c *gc.C) {
	...
	id, err := s.client.UploadCharm(
		...
		entitytesting.Repo.CharmDir("wordpress"),
	)
...
func (s *suite) TestUploadBundleErrorUploading(c *gc.C) {
	...
	id, err := s.client.UploadBundle(
		...
		entitytesting.Repo.BundleDir("wordpress-simple"),
	)
